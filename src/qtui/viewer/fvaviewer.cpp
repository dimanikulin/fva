...
FVAViewer::FVAViewer(const QString& rootDir, const QString& dictPath, QWidget* parent, Qt::WindowFlags /*flags*/)
    : QDialog(parent), m_ui(new Ui::FVAViewerClass), m_dictionaryPath(dictPath) {
    m_ui->setupUi(this);
    ...
    connect(m_ui->treeWidget, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this,
            SLOT(editFileItem(QTreeWidgetItem*)));
...
void FVAViewer::updateChecks(QTreeWidgetItem* item, int column) { fvaUpdateChecks(item, column); }
FVAViewer::~FVAViewer() {
    delete m_ui;
...
void FVAViewer::showItem(QTreeWidgetItem* item) {
    ...
    if (!fvaitem) return;
    setWindowTitle(fvaitem->getGuiName(m_dictionaries) + " [" + fvaitem->getGuiFullName(m_dictionaries) + "] (" +
                   fvaitem->fsFullPath + ")");
...
| QDir::Files, QDir::DirsFirst))
        {
                if ( info.isDir() )
                {
                        ...
                        {
                                dirItem->placeId				=	result["place"]
.toUInt(); dirItem->peopleIds				=	fvaStringToIds(result["people"].toString());
                                dirItem->deviceId				=	result["deviceId"]
.toUInt(); dirItem->eventId				=	result["event"]			.toUInt();
                                dirItem->eventReasonPeopleIds	=
fvaStringToIds(result["reasonPeople"].toString()); dirItem->tagsOrComment			=	result["tags"]
.toString(); dirItem->linkedFolder			=	result["linkedFolder"]	.toString();
                ...
                else
                {
                        ...
                        if (!fvaitem->decsItems.isEmpty() && !fvaitem->descTitles.isEmpty())
                        {
                                ...
                                if ( it != fvaitem->decsItems.end() )
                                {
                                        ...
                                        int columnId =
FVADescriptionFile::getColumnIdByName(fvaitem->descTitles,"Comment"); if ( -1 != columnId ) fileItem->tagsOrComment
= list[columnId].remove("\t");
                                        ...
                                        columnId =
FVADescriptionFile::getColumnIdByName(fvaitem->descTitles,"Description"); if ( -1 != columnId ) fileItem->description
= list[columnId].remove("\t");
...
void FVAViewer::fvaFilterTree(const fvaFilter& filter, fvaItem* fvaitem, const QDateTime& defFilterDataTime) {
    for (auto idChild = fvaitem->children.begin(); idChild != fvaitem->children.end(); ++idChild) {
        ...
        if (defFilterDataTime != filter.dateFrom && defFilterDataTime != filter.dateTo) {
            if ((*idChild)->type == FVA_FS_TYPE_DIR) {
