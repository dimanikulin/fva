...
FVAOrganizerEventInfoPage::FVAOrganizerEventInfoPage(void) {
    ...
    words = new QLabel(
        tr("Please select a folder bellow (an event),marked by red\nand choose event type and related people."));
...
void FVAOrganizerEventInfoPage::updateChecks(QTreeWidgetItem *item, int column) { fvaUpdateChecks(item, column); }
...
bool FVAOrganizerEventInfoPage::validatePage() {
    ...
    QString inputDir = ((FVAOrganizerWizard *)wizard())->inputFolder();
    FVA_EXIT_CODE exitCode = flow.ProcessInputDirForEvents(inputDir.toStdString(), dir2EventMap, dir2PeopleMap, this);
