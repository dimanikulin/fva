...
FVADescriptionEditor::FVADescriptionEditor(bool forFolder, QWidget* parent)
    : QMainWindow(parent), m_forFolder(forFolder) {
...
FVADescriptionEditor::~FVADescriptionEditor() { LOG_DEB << "destructed"; }

void FVADescriptionEditor::updateChecks(QTreeWidgetItem* item, int column) { fvaUpdateChecks(item, column); }
...
+ "\",\n"; content["people"] = peopleIDs; if (!place.isEmpty())
                        // jsonData += "\"place\":\""	+ place
+ "\",\n"; content["place"] = place; if (!event.isEmpty())
                        // jsonData += "\"event\":\""	+ event
+ "\",\n"; content["event"] = event; if (!linkedPeopleIDs.isEmpty())
                        // jsonData += "\"reasonPeople\":\""	+ linkedPeopleIDs
+ "\",\n"; content["reasonPeople"] = linkedPeopleIDs; if (!ui.editOldName->text().remove("\t").isEmpty())
                ...
                QString error;
                FVA_EXIT_CODE res = fvaCreateFolderDescription( m_folderPath + "/" + FVA_DIR_DESCRIPTION_FILE_NAME,
content, error ); RET_IF_RES_NO_ERROR
        }
        else
        {
                ...
                QStringList& list = it.value();
                list[FVADescriptionFile::getColumnIdByName(m_titles,"oldName")]		=
ui.editOldName->text().remove("\t"); list[FVADescriptionFile::getColumnIdByName(m_titles,"Description")] =
ui.editDescription->text().remove("\t"); list[FVADescriptionFile::getColumnIdByName(m_titles,"Comment")] =
ui.editComment->text().remove("\t"); int index = -1;
