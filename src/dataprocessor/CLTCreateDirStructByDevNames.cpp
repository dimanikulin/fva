...
FVA_EXIT_CODE CLTCreateDirStructByDeviceName::execute(const CLTContext& /*context*/) {
    ...
    Q_FOREACH (QFileInfo info,
        ...
        if (FVA_FS_TYPE_IMG == fvaConvertFileExt2FileType(suffix.toStdString())) {
            ...
            while (newDeviceName.endsWith('\r')) newDeviceName.chop(1);

            while (newDeviceName.endsWith(QChar('\0'))) newDeviceName.chop(1);
            ...
            if (newDeviceName.isEmpty()) {
                ...
                if (deviceName.isEmpty())
                    ...
                    LOG_WARN << "got new device name: " << newDeviceName << "in picture:" << info.absoluteFilePath()
                             << "old: " << deviceName;
