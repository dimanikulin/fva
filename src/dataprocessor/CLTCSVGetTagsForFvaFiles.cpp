...
CLTCSVGetTagsForFvaFiles::CLTCSVGetTagsForFvaFiles(const FvaConfiguration& cfg) {
    ...
    if (res != FVA_NO_ERROR)
        LOG_CRIT << "Failed to fvaLoadSimpleMapFromCsvByItemType fvaPlaceTypes.csv with erorr - " << res;
    RET_IF_RES_IS_ERROR
    ...
    if (res != FVA_NO_ERROR)
        LOG_CRIT << "Failed to fvaLoadSimpleMapFromCsvByItemType fvaRelationTypes.csv with erorr - " << res;
    RET_IF_RES_IS_ERROR
...
FVA_EXIT_CODE CLTCSVGetTagsForFvaFiles::getFvaTagsForFile(const QString& fileName, QString& tags) {
    ...
    if (m_SearchByPlace && fvaFileItem.placeId != 0 && fvaFileItem.placeId != FVA_UNDEFINED_ID) {
        ...
        tags += TagTypeDelim + m_fvaTagsTypeMap[1] + TagDelim + itPlaceType.value() + TagDelim + itPlace.value().name;
    }
    ...
    if (m_SearchByEvent && fvaFileItem.eventId != 0 && fvaFileItem.eventId != FVA_UNDEFINED_ID) {
        ...
        tags +=
            TagTypeDelim + m_fvaTagsTypeMap[3] + TagDelim + itRelationTypes.value() + TagDelim + itEvent.value().name;
