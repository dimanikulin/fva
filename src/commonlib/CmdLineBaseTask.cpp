...
FVA_EXIT_CODE CmdLineBaseTask::processFolderRecursivly(const std::string& folder, const CLTContext& context) {
    ...
    Q_FOREACH (QFileInfo info,
               recurDir.entryInfoList(QDir::NoDotAndDotDot | QDir::System | QDir::Hidden | QDir::AllDirs | QDir::Files,
                                      QDir::DirsFirst)) {
        ...
        if (info.isDir()) {
            ...
            FVA_EXIT_CODE res =
                processFolderRecursivly(QDir::toNativeSeparators(info.filePath()).toStdString(), context);
