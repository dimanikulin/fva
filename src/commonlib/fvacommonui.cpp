...
void fvaPopulateInputDir(const QString& folder, QTreeWidgetItem* item, QTreeWidget* treeWidget,
    ...
    Q_FOREACH (QFileInfo info,
        ...
        if (info.fileName()[0] == '#' && info.fileName()[info.fileName().size() - 1] == '#') continue;
        ...
        if (!info.isDir())  // now it is a file
        {
            for (auto it = problemFileList.begin(); it != problemFileList.end(); ++it) {
                QString fileName = QString::fromStdString(*it);
...
FVA_EXIT_CODE fvaBuildSimpleTree(QWidget* pMainWnd, QTreeWidget* pTreeWidget, const std::string& rootSWdir,
    ...
    for (auto i = firstLvlMap.begin(); i != firstLvlMap.end(); ++i) {
        int ID = i->first;
        QTreeWidgetItem* treeWidgetItem = new QTreeWidgetItem;
        treeWidgetItem->setText(0, QString::fromStdString(i->second));
        ...
        for (auto index = secondLvlMap.begin(); index != secondLvlMap.end(); ++index) {
            ...
            childWidgetItem->setText(0, QString::fromStdString(index->second));
...
void fvaClearChecks(QTreeWidgetItem* item) {
    ...
    if (item->childCount()) {
        for (int i = 0; i < item->childCount(); ++i) fvaClearChecks(item->child(i));
    }
}
void fvaUpdateChecks(QTreeWidgetItem* item, int column) {
    ...
    if (item->childCount() != 0 && item->checkState(0) != Qt::PartiallyChecked && column != -1) {
        ...
        for (int i = 0; i < item->childCount(); ++i) item->child(i)->setCheckState(0, checkState);
        ...
        if (item->parent() != 0) fvaUpdateChecks(item->parent(), -1);
    }
}
