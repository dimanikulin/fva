...
int FVADescriptionFile::getColumnIdByName(const QStringList& titles, const QString& columnName) {
    for (int id = 0; id < titles.size(); ++id) {
        if (columnName.toUpper() == titles[id].toUpper()) return id;
    }
    return -1;
}
FVA_EXIT_CODE FVADescriptionFile::save(const QString& path, const QStringList& titles,
    ...
    for (auto i = decsItems.begin(); i != decsItems.end(); ++i)
        if (i.value().size() != titles.size()) return FVA_ERROR_INCORRECT_FORMAT;
