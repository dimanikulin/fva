...
FVA_FS_TYPE fvaConvertFileExt2FileType(const QString& extention) {
    if (extention.toUpper() == "JPG" || extention.toUpper() == "JPEG"
    ...
    if (extention.toUpper() == "AVI" || extention.toUpper() == "MOV" || extention.toUpper() == "MPG" ||
        extention.toUpper() == "MP4" || extention.toUpper() == "3GP" || extention.toUpper() == "MKV")
        return FVA_FS_TYPE_VIDEO;

    if (extention.toUpper() == "WAV") return FVA_FS_TYPE_AUDIO;
...
FVA_EXIT_CODE fvaRunCLT(const QString& cmdName, const QString& inputDir, bool isRecursive, bool isReadOnly,
    ...
    if (!custom.isEmpty()) params.append("custom=" + custom);
...
FVA_EXIT_CODE fvaSaveStrListToFile(const QString& path, const QList<QString>& strList) {
    ...
    for (auto it = strList.begin(); it != strList.end(); ++it) writeStream << *it << "\n";
    writeStream.flush();
    fileNew.close();
